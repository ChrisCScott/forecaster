...
from forecaster import Person, Debt
...
class TestDebtMethods(unittest.TestCase):
    ...
    def setUp_decimal(self):
        """ Sets up class attributes with Decimal inputs. """
        super().setUp()
        # We use caps because this is a type.
        # pylint: disable=invalid-name
        self.AccountType = Debt
        ...
        # It's important to synchronize the initial years of related
        # objects, so store it here:
        self.initial_year = Decimal(2000)
        # Every init requires an owner, so store that here:
        self.owner = Person(
            self.initial_year, "test", Decimal(2000),
            raise_rate={year: Decimal(1) for year in range(2000, 2066)},
            retirement_date=Decimal(2065),
    ...
    def test_init_default(self):
        ...
        account = self.AccountType(
            self.owner)
        self.assertEqual(account.minimum_payment, 0)
        self.assertEqual(account.accelerated_payment, float('inf'))

    def test_init_explicit(self, *args, **kwargs):
        """ Test Debt.__init__ with explicit args of expected types. """
        minimum_payment = 100
        accelerated_payment = 0
        account = self.AccountType(
            self.owner, *args,
    ...
    def test_decimal(self):
        """ Test Debt with Decimal inputs. """
        # Convert values to Decimal:
        self.setUp_decimal()
        # This is based on test_max_inflows_large_balance:
        self.debt.minimum_payment = Decimal(100)
        self.debt.balance = Decimal(-1000)
        ...
        # Confirm the result is Decimal-typed and correct:
    ...
    def test_max_inflows_small_balance(self):
        ...
        self.debt.balance = -100
        result = self.debt.max_inflows(self.timing)
        for when, value in result.items():
            self.debt.add_transaction(value, when=when)
        # Result of `max_outflows` should bring balance to $0 if
        # applied as transactions:
        self.assertAlmostEqual(self.debt.balance_at_time('end'), 0)
    ...
    def test_max_inflows_large_inflow(self):
        ...
        result = self.debt.max_inflows(self.timing)
        target = 0  # We can't add any more
