...
class Account(TaxSource, MoneyHandler):
    ...
    Attributes:
        ...
        rate (Real): The rate of return (or interest) for this year,
            before compounding.
        rate_history (dict[int, Real]): `{year: rate}` pairs covering
        ...
        transactions (dict[Real, Money]): The transactions to/from
        ...
        transactions_history (dict[int, dict[Real, Money]]):
    ...
    def __init__(
        ...
        if initial_year is None:
            if hasattr(owner, 'initial_year'):
                initial_year = owner.initial_year  # type: ignore[union-attr]
            else:
                raise TypeError(
        ...
        self._transactions: Transactions = defaultdict(
            lambda: self.money_factory(0))
        # These two are set via assignment to corresponding properties:
        self._rate_callable: Callable[[int], Real]
    ...
    @owner.setter
    def owner(self, val: Optional[Person]) -> None:
        ...
        if val is not None:
            if hasattr(val, 'accounts'):
                # Register with new owner:
                val.accounts.add(self)
            else:
                raise AttributeError(
                    'Account: owner must have `accounts` attribute.')
    ...
    def default_timing(self, val: Union[Timing, Dict[Real, Real]]) -> None:
        """ Sets default_timing. """
        # Cast to `Timing` type:
    ...
    def balance(self) -> MoneyType:
        ...
        balance = value_at_time(
            ...
            # Pylint gets confused by attributes added by metaclass.
            self._balance_history[  # type: ignore[attr-defined]
                self.this_year - 1],
        ...
        transactions_history = (
            self._transactions_history)  # type: ignore[attr-defined]
    ...
    @property
    def rate_callable(self) -> Callable[[int], Real]:
    ...
    def rate_callable(
            self, val: Union[Callable[[int], Real], Dict[int, Real], Real]
        ) -> None:
        """ Sets rate_callable.
        ...
        if not callable(val):
            if isinstance(val, dict):
                ...
                # For scalar input, return the same value every year:
                def func(year):  # pylint: disable=unused-argument
    ...
    def max_outflow(self, when: Time = "end") -> Optional[MoneyType]:
        ...
        return max(
            ...
            min(-self.balance_at_time(when), self.money_factory(0)),
    ...
    def transactions_to_balance(
        ...
        * Each `value` is proportionate to the corresponding
        ...
        Arguments:
            ...
            transactions (dict[Real, Money]): If provided, the result
                of this method will be determined as if the account
                also had these transactions recorded against it.

        Returns:
            dict[Real, Money]: A mapping of `{when: value}` pairs where
        ...
        weighted_accum = 0
        for when, weight in normalized_timing.items():
    ...
    def max_outflows(
        ...
        if self.max_outflow_limit is None:  # No limit on outflows:
            min_total = transaction_limit
        else:  # Limit on outflows, which may be further reduced:
            # This method determines how much more can be withdrawn,
            # so if there's a limit on total withdrawals find out how
            # much withdrawal room is left after current withdrawals:
    ...
    def __iter__(self) -> Iterator[Time]:
        for when in sorted(self._transactions.keys()):
            yield when

    def __contains__(self, key: Real) -> bool:
        when = when_conv(key)
    ...
    def balance_at_time(
        ...
        Returns:
            (Money) The balance at time `time`.
        """
