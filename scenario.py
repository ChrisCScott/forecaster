''' Basic economic classes, such as `Scenario` and `Money`. '''
import collections
from numbers import Number
from settings import Settings

# A `ScenarioYear` describes inflation, stock_return, and bond_return
ScenarioYear = collections.namedtuple("ScenarioYear",
                                      "inflation stock_return bond_return")


class Scenario(collections.Sequence):
    ''' Describes an economic scenario over the course of the simulation.

    For example, provides inflation rates and rates of return for each
    year of the simulation. This is the foil to `Strategy`; all
    `Scenario` information is reflective of broader economic trends and
    is independent of any user action.

    A `Scenario` is immutable.
    '''

    def __init__(self, inflation, stock_return, bond_return,
                 initial_year=None):
        ''' Constructor for `Scenario`.

        Arguments must be lists (or similar `Sequence`) with matching
        lengths. Each element corresponds to a year, starting with
        `initial_year`. If `initial_year` is not provided, the object
        will use `Settings.initialYear`.

        Args:
            inflation (list): A list of inflation values, one for each
                year of the simulation.
            stock_return (list): A list of stock return values, of the
                same length as `inflation`.
            bond_return (list): A list of bond return values, of the
                same length as `inflation` and `stock_return`.
            initial_year (int): The first year of the simulation.

        Raises:
            TypeError: Non-list or non-int provided as input.
            ValueError: Input lists not of matching lengths.
        '''
        # TODO: Allow for scalar, list, or dict inputs.
        # Implement via defaultdict class.
        # Allow caller to pass a Settings object and use that for
        # default values. If None, use the class values for Settings
        # This should allow for deleting the ConstantScenario and
        # DefaultScenario classes entirely, since defaultdict is
        # lightweight and efficient.
        if not (isinstance(inflation, collections.Sequence) and
                isinstance(stock_return, collections.Sequence) and
                isinstance(bond_return, collections.Sequence)):
            raise TypeError(
                "Scenario: Unexpected non-sequence object passed to __init__")
        if not (len(inflation) == len(stock_return) and
                len(inflation) == len(bond_return)):
            raise ValueError(
                "Scenario: Input lists are not of matching lengths")
        if not (initial_year is None or isinstance(initial_year, Number)):
            raise TypeError("Scenario: initial_year must be numeric")
        self._years = {}
        if initial_year is None:
            initial_year = Settings.initial_year
        keys = range(initial_year, len(inflation))
        vals = [ScenarioYear(*x) for x in
                zip(inflation, stock_return, bond_return)]
        self._years = dict(keys, vals)

    def initial_year(self):
        ''' The initial year. '''
        return self._years.keys()[0]

    def discount_rate(self, year):
        ''' Returns the discount rate for `year`.
        This is the same as calling `inflation(year)` '''
        return self[year].inflation

    def accumulation_function(self, year1, year2):
        ''' Returns the discount to be applied over the period from
        `year1` to `year2`. If `year1 > year2` then the discount rate is
        inverted. '''

        # TODO: Cache list of accumulations from Settings.displayYear
        # to each other year? This method gets a lot of use, so it would
        # be more efficient. But remember to check for changes to
        # Settings.displayYear. '''
        accum = 1
        if year1 <= year2:
            # Find the product of all intervening years' discount rates
            for year in range(year1, year2):
                accum = accum*(1+self.discount_rate(year))
            return accum
        else:  # Same as above, except invert the result ()
            for year in range(year2, year1):
                accum = accum*(1+self.discount_rate(year))
            return 1/accum

    def real_value(self, value, nominal_year, real_year=None):
        ''' Returns a value expressed in `real_year` terms.

        If `real_year` is not provided, uses the current display year
        (see `Settings`). '''
        if real_year is None:
            real_year = Settings.display_year
        discount = self.accumulation_function(nominal_year, real_year)
        return value*discount

    # Implement collections.Sequence methods.
    def __len__(self):
        ''' Returns the number of years modelled by the `Scenario`. '''
        return len(self._years)

    def __getitem__(self, year):
        ''' `self[year]` returns a `ScenarioYear` tuple for `year`.
        (This is the same as `self.inflation(year)`) '''
        return self._years[year]

    def __contains__(self, val):
        return val in self._years

    def __iter__(self):
        for year in self._years:
            yield year

    def __reversed__(self):
        for year in reversed(self._years):
            yield year

    def index(self, val, start=None, stop=None):
        return self._years.index(val, start, stop)

    def count(self, val):
        return self._years.count(val)

    def values(self):
        return self._years


class ConstantScenario(Scenario):
    ''' A simplified `Scenario` where values are the same for each year.

    Useful for setting economic assumptions for simple tests.'''
    def __init__(self, inflation, stock_return, bond_return,
                 initial_year=None):
        ''' Constructor for `ConstantScenario`. Arguments must be numeric.
        The same numeric value is used for each year.` '''
        if not isinstance(inflation, Number) or \
           not isinstance(stock_return, Number) or \
           not isinstance(bond_return, Number) or \
           (not isinstance(initial_year, Number) and
           initial_year is not None):
            raise TypeError(
                "ConstantScenario: Unexpected non-numeric argument")
        self._year = ScenarioYear(inflation, stock_return, bond_return)
        self._initial_year = initial_year
        self._num_years = Settings.num_years

    def initial_year(self):
        return self._initial_year

    def accumulation_function(self, year1=None, year2=None):
        ''' Returns the discount to be applied from `year1` to `year2`.

        If `year1 > year2` then the discount rate is inverted. '''
        return pow(1+self._year.inflation, year2-year1)

    # Implement collections.Sequence methods:
    def __len__(self):
        ''' Returns `Settings.num_years` '''
        return Settings.num_years

    def __getitem__(self, year):
        ''' `self[year]` returns a `ScenarioYear` tuple for `year`. '''
        return self._year

    def __contains__(self, val):
        return val == self._year

    def __iter__(self):
        for i in self._num_years:
            yield self._year

    def __reversed__(self):
        for i in self._num_years:
            yield self._year

    def index(self, val, start=None, stop=None):
        if val == self._year:
            return 0
        else:
            raise ValueError("ConstantScenario.index: Value not found")

    def count(self, val):
        if val == self._year:
            return self._num_years
        else:
            return 0

    def values(self):
        return [self._year] * self._num_years


class DefaultScenario(ConstantScenario):
    ''' A constant `Scenario` that always returns the corresponding values of
    `Settings.DefaultScenario` '''

    def __init__(self):  # pylint: disable=W0231
        ''' Constructor for `DefaultScenario`.

        Doesn't call superclass methods to avoid creating attributes '''
        pass

    @property
    def initial_year(self):
        ''' Returns `Settings.initial_year` '''
        return Settings.initial_year

    @property
    def _year:
        ''' Returns `Settings.ScenarioDefaults.inflation` '''
        return ScenarioYear(Settings.ScenarioDefaults.inflation,
                            Settings.ScenarioDefaults.stock_return,
                            Settings.ScenarioDefaults.bond_return)

default_scenario = DefaultScenario()
