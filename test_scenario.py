...
class TestScenarioMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.constant_scenario = Scenario(self.constant_inflation, self.constant_stock_return,
                                          self.constant_bond_return, self.constant_initial_year)

        # Set up a `Scenario` with varying elements, including at least one 0 in each list
        ...
        self.varying_scenario = Scenario(self.varying_inflation, self.varying_stock_return,
        ...
        for _ in range(100): # add 100 random scenarios to the standard test set, for safety's sake
            self.scenarios.append(self.get_random_scenario())

    def scenario_test(self, inflation, stock_return, bond_return, length,
                      initial_year=None):
        ''' Builds a `Scenario` object based on input arguments and runs various tests.
        Returns the object for further testing. A utility function for `test_init`. '''
        scenario = Scenario(inflation, stock_return, bond_return, initial_year)
        self.assertIsNotNone(scenario) # construction didn't fail
        self.assertEqual(len(inflation), len(scenario)) # all years are present
        ...
        for i in range(0, length):
            self.assertEqual(scenario.inflation(i + initial_year), inflation[i])
            self.assertEqual(scenario.stock_return(i + initial_year), stock_return[i])
            self.assertEqual(scenario.bond_return(i + initial_year), bond_return[i])
        return scenario # Return scenario for further testing

    def type_test(self, type_, inflation, stock_return, bond_return, initial_year=None):
        ...
        for year in range(initial_year, initial_year + len(scenario)):
            self.assertEqual(scenario.inflation(year), type_(inflation[year - initial_year]))
            self.assertEqual(scenario.stock_return(year), type_(stock_return[year - initial_year]))
            self.assertEqual(scenario.bond_return(year), type_(bond_return[year - initial_year]))

    def test_init(self):
        ''' Tests `Scenario.__init__()`. Also tests `inflation()`, `stock_return()`, and
        `bond_return()` '''
        ...
        self.type_test(Decimal, self.varying_inflation, self.varying_stock_return,
                       self.varying_bond_return, self.varying_initial_year)
        self.type_test(float, self.varying_inflation, self.varying_stock_return,
                       self.varying_bond_return, self.varying_initial_year)
        self.type_test(int, self.varying_inflation, self.varying_stock_return,
                       self.varying_bond_return, self.varying_initial_year)
    ...
    def test_accumulation_function(self):
        ...
        for year in range(Settings.initial_year, Settings.initial_year + self.constant_length):
            self.assertAlmostEqual(scenario.accumulation_function(self.constant_initial_year, year),
        ...
        for scenario in self.scenarios:
            accum = 1
            for year in range(scenario.initial_year(), scenario.initial_year() + len(scenario)):
                self.assertAlmostEqual(scenario.accumulation_function(scenario.initial_year(),
                                                                      year), accum)
    ...
    def test_list_methods(self):
        ''' Tests `Scenario.__len__`, `Scenario.__getitem__`, and `ScenarioYear` '''
        scenario = self.constant_scenario
        length = self.constant_length
        inputs = (self.constant_inflation, self.constant_stock_return, self.constant_bond_return)
        initial_year = self.constant_initial_year
        for array in inputs:
            self.assertEqual(len(scenario), length)
            self.assertEqual(len(scenario), len(array))
        for year in range(initial_year, initial_year + length): # test as tuple and individually
            self.assertEqual(scenario[year], ScenarioYear(inputs[0][year - initial_year],
                                                          inputs[1][year - initial_year],
                                                          inputs[2][year - initial_year]))
            self.assertEqual(scenario[year].inflation, inputs[0][year - initial_year])
            self.assertEqual(scenario[year].stock_return, inputs[1][year - initial_year])
            self.assertEqual(scenario[year].bond_return, inputs[2][year - initial_year])


        scenario = self.varying_scenario
        length = self.varying_length
        inputs = (self.varying_inflation, self.varying_stock_return, self.varying_bond_return)
        ...
        for scenario in self.scenarios:
            for year in range(initial_year, initial_year + len(scenario)):
                self.assertEqual(scenario[year].inflation, scenario.inflation(year))
                self.assertEqual(scenario[year].stock_return, scenario.stock_return(year))
                self.assertEqual(scenario[year].bond_return, scenario.bond_return(year))
